...
plot_ww <- function() {
  ...
  WW_all_long_sHB <- WW_all_long_sHB %>%
    filter(parameter %in% c("F","sHM"))
  ...
  png(file.path("plots","out","Figure_SI_1_2-panel_presentation.png"),res = 300,height = 7, width = 6, units = "in")
  print(x = ww_plot)
  dev.off()
