...
modeling_summary_table <- function() {
  ...
  MMSD_models_wide <- pivot_wider(data = MMSD_models_3_site,names_from = response, values_from = c(model,rmse_median))
  ...
  ##########  ERRORR BELOW. ALSO NEED TO COMBINE 2-SITE AND 3-SITE RESULTS
  ...
  MMSD_models_wide_3-site <- MMSD_models_wide
  ...
  site_combos <- unique(MMSD_2_site$site_combo)
  response <- levels(MMSD_2_site$response)
  ...
  names(MMSD_models_wide) <- MMSD_names
  MMSD_models_wide_2-site <- MMSD_models_wide
  
  MMSD_models <- as.data.frame(rbind(GLRI_LMER_models,GLRI_OLS_models)) %>%
    #  mutate(organism = paste(response,model_run,sep="_")) %>%
    select(site_combo,model_run,model,response,rmse_median)
  
  MMSD_models_wide <- full_join(MMSD_models_wide_3-site,MMSD_models_wide_2_site)
