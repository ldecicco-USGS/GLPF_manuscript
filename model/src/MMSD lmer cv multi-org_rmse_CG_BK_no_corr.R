...
for(i in 1:length(sensors)) {
  form[[(i)]] <- formula(paste("log_response ~ ",paste(sensors[i],c("* sinDate","* cosDate"),collapse=" + "),"+ sinDate + cosDate + (1 | abbrev)"))
  form_names <- c(form_names,sensors[i])
}
...
for (s in 1:length(site_combos)) {  #Solo JI doesn't need lmer, but just lm
  ...
  for (i in 1:length(response)) {
    ...
    filenm <- paste("MMSD_Oct_12_2020_CG_BK_no_corr",names(site_combos)[s],"_",response[i],".pdf",sep="")
  ...
  filenm <- file.path("model","out",paste("rmse_Oct_11_no_corr",names(site_combos)[s],".rds",sep=""))
  saveRDS(rmse_df, file = filenm)
  
  filenm <- file.path("model","out",paste("rmse_and_sites_Oct_11_no_corr",names(site_combos)[s],".rds",sep=""))
