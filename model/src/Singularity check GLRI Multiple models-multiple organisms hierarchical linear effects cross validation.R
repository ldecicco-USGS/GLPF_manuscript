...
form_names <- c("F,T","F,Turb","T,Turb","F","T","Turb","F,Aresid","T,S1","F,A254","F,T,Turb", "F,T,Turb 2")
form <- list()
form[[1]] <- formula("log_response ~ F * cosDate + T * cosDate + F * sinDate + T * sinDate + (F + 1 | abbrev)")
form[[2]] <- formula("log_response ~ F * cosDate + Turbidity_mean * cosDate + F * sinDate + Turbidity_mean * sinDate + (F + 1 | abbrev)")
form[[3]] <- formula("log_response ~ T * cosDate + Turbidity_mean * cosDate + T * sinDate + Turbidity_mean * sinDate + (T + 1 | abbrev)")
form[[4]] <- formula("log_response ~ F * cosDate + F * sinDate + sinDate + cosDate + (F + 1 | abbrev)")
...
form[[8]] <- formula("log_response ~ T * cosDate + T * sinDate + S1.25 * cosDate + S1.25 * sinDate  + (S1.25 | abbrev)")
form[[9]] <- formula("log_response ~ F * cosDate + F * sinDate + A254 * cosDate + A254 * sinDate  + (A254 + F | abbrev)")
form[[10]] <- formula("log_response ~ F * cosDate + F * sinDate + T * cosDate + T * sinDate  + Turbidity_mean * cosDate + Turbidity_mean * sinDate + (F + Turbidity_mean | abbrev)")
form[[11]] <- formula("log_response ~ F * cosDate + F * sinDate + T * cosDate + T * sinDate  + Turbidity_mean * cosDate + Turbidity_mean * sinDate + (F | abbrev)")
...
# 
# 
# m <- lmer(form[[i]],data=model_df_scaled); i
# i <- i+1;i
# 
...
for (s in 1:length(site_combos)) {
  ...
  sites <- site_combos[[s]]
    ...
    filenm <- paste("singularity_check_GLRI_model_options_test",names(site_combos)[s],"_",response[i],".pdf",sep="")
    filenm <- file.path("model","out","plots",filenm)
    ggexport(multi.page, filename = filenm,width = 11,height = 8)
    
    }
  saveRDS(rmse_df, file = paste("Singularity_check_rmse_",names(site_combos)[s],".rds",sep=""))
