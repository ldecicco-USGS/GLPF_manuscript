...
for (s in 1:length(site_combos)) {
  ...
  sites <- site_combos[[s]]
    ...
    filenm <- paste("GLRI_predictions_test",response[i],".pdf",sep="")
    #  pdf(filenm)
    ...
    filenm <- paste("GLRI_model_options_test",names(site_combos)[s],"_",response[i],".pdf",sep="")
    filenm <- file.path("model","out","plots",filenm)
    ggexport(multi.page, filename = filenm,width = 11,height = 8)
    
    }
  filenm <- file.path("model","out",paste("rmse_test",names(site_combos)[s],".rds",sep=""))
