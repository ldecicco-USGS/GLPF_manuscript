...
names(site_combos) <- c("CL_JI","Agricultural")
...
#length(site_combos)) {  #Solo JI doesn't need lmer, but just lm  
...
for (s in (1:length(site_combos))) { 
  ...
  for (i in 1:length(response)) {
    ...
    nrows <- 5
    ncols <- 4
    pages <- ceiling(length(form)/(nrows*ncols))
    
    model_plot <- list()
    for(p in 1:pages) {
      model_plot[[p]] <- ggplot(data = model_results_df,aes(x = observed,y = predicted, color=abbrev)) + 
        geom_point() +
        ...
        facet_wrap_paginate(~ model_name,ncol = ncols, nrow = nrows,page=p)
      #      facet_wrap(~ model_name) 
    }
    
    #model_plot  
    model_plot[[p+1]] <- rmspeboxplot
    multi.page <- ggarrange(plotlist = model_plot, #rmspeboxplot) #,
                            nrow = 1, ncol = 1)
    
    filenm <- paste("GLRI_Sep_27_non_corr_",names(site_combos)[s],"_",response[i],".pdf",sep="")
    filenm <- file.path("model","out","plots",filenm)
    ggexport(multi.page, filename = filenm,width = 11,height = 8)
    
  }
  filenm <- file.path("model","out",paste("rmse_non_corr_",names(site_combos)[s],".rds",sep=""))
