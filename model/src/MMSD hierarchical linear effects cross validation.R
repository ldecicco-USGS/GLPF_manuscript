...
source(file.path("model","src","plot_model_cv_MMSD.R"))
...
predictors<- c("Aresid267", "T", "F","rS1.25_T","S1.25")
...
form[[2]] <- formula("log_response ~ F * cosDate + Aresid267 * cosDate + F * sinDate + Aresid267 * sinDate + (F + Aresid267 | abbrev)")
form[[3]] <- formula("log_response ~ F * cosDate + F * sinDate + (F | abbrev)")
form[[4]] <- formula("log_response ~ T * cosDate + T * sinDate  + (T | abbrev)")
form[[4]] <- formula("log_response ~ Aresid267 * cosDate + Aresid267 * sinDate  + (Aresid267 | abbrev)")
form[[5]] <- formula("log_response ~ rS1.25_T * cosDate + rS1.25_T * sinDate  + (rS1.25_T | abbrev)")
form[[6]] <- formula("log_response ~ T * cosDate + T * sinDate + rS1.25_T * cosDate + rS1.25_T * sinDate  + (rS1.25_T + T | abbrev)")
form[[7]] <- formula("log_response ~ T * cosDate + T * sinDate + S1.25 * cosDate + S1.25 * sinDate  + (S1.25 + T | abbrev)")

# 3. Run LME model for all response variables

for (i in 1:length(response)) {
  
  filenm <- paste("MMSD_predictions_cv_",response[i],".pdf",sep="")
  pdf(filenm)
  ...
  filenm <- paste("running_mean_rmspe_MMSD_",response[i],".pdf",sep="")
  pdf(filenm)
  for(f in 1:length(form)) {
  ...
  filenm <- paste("model_options_MMSD_",response[i],".pdf",sep="")
  model_AICs <- numeric()
  pdf(filenm)
  for(f in 1:length(form)){
