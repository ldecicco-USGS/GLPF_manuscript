...
## Regression Modeling
The first consideration in regression modeling was the choice of sites to include in common models. Watershed and subwatershed sites were initially grouped together for a unified model and small scale sites were grouped together. Next, categories from watershed and subwatershed sites were chosen for similarity in land cover and potential diversity of water quality influences as well as geographic proximity to minimize differences in background organic carbon that could lead to confounding signals in these regression relations. This resulted in four site categories; watershed sites were separated into three categories for model development including Agricultural (Maumee, Portage, and Raisin), Urban (Clinton and Rouge), and single site (Milwaukee), and the three subwatershed sites were included as the fourth category. Separating sites into these four categories resulted in improved cross-validated predictive capability for resulting models over the watershed- and subwatershed-scale unified model, so these four categories were chosen for inclusion in final model selection (Table \@ref(tab:tabTest)). 

Model development attempts for the small-scale sites included one unified model for all samples including all sites within the three regions, and separate models for each of the three regions. These attempts to develop models for the small-scale sites resulted in reasonable regressions when considering the fit for these overall data sets, but when put in context of the primary objective to identify optical signals for identifying contamination sources, no adequate models were discovered. To meet sensor development needs, regressions must provide increasing predictions for increasing human bacteria concentrations for individual events within a storm sewer system or small drainage basin. In order for the In order did not result in acceptable models. 

when considering the objective of identifying optical signals for sensor development, did not prove to be adequate.

did not reliably provide increasing predictions for increasing human bacteria observations for individual events within individual drainage basins. 
...
attempts for small scale included
 -unified model
 -separate model for each region
 All resulting model attempts resulted in unreasonable models
  -overall models appeared to have a reasonable relation between optical signals and human bacteria,
  -The objective is to develop a sensor that can to track contamination back to the source. To meet this objective, the regression must reliably predict an increasing signal with increasing human bacteria concentration for individual events within an individual storm sewer drainage basin. Examining the regressions in this context revealed that relations did not 
  -but when examined by region and individual event, cross validated errors indicated that the resulting regressions were not of use for predicting human bacteria from optical signals.




to provide the best chance of a consistent relation between organic carbon characterization and sewage contamination. The Maumee River, Portage River, and River Raisin are agricultural tributaries of Western Lake Erie, The Clinton River and River Rouge are watersheds with a high degree of urban influence within the Lake St. Claire-Detroit River corridor, the Milwaukee River was sampled in an area with a regular seiche effect that includes transitional influences from the Milwaukee River and Lake Michigan, and the subwatershed sites are at different locations within the same watershed. 
