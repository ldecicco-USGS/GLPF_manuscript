...
Regression modeling was used to define relations between bacteria concentrations (response variables) and optical signals (explanatory variables). From a practical standpoint for potential field application, variable selection for these regressions was done in three **(CHANGE TO TWO???)** potential stages: First, optical signals that were already available as field sensors were considered first (case 1). Next, if reasonable relationships could not be determined in case 1, optical signals that are in a reasonable excitation/emmission range for development of custom field sensors were then considered (case 2). When the first two cases did not result in reasonable results, explanatory variables were expanded to use of numerous additional optical signals or variables that could be derived from optical signals within the data set.
