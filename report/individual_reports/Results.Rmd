...
```{r VirusOccurrenceFig, fig.width=3,fig.height=5, fig.cap = "Fraction of human virus occurrence as compared to concentration bins for A. the sum of human associated bacteria, B. Escherichia coli, and C. Enterococci in samples from subwatersheds of the Milwaukee River (subwatershed scale), and tributaries of the Great Lakes (watershed scale). Number of observations in each concentration bin are profided above the bars. sHM concentration of 450 Copy Number/100mL represents the detection level. E. coli concentration of 235 CFU/100mL represents a common recreational water quality criteria.", echo=FALSE}
...
Models were developed with two sets of optical variables: First, using optical signals represent those used in commercially available field sensors (sensor signals), and second, using additional optical signals that were not highly correlated with at least one of the sensor signals (non-correlated signals). There was not substantial improvement in cross-validation error (root mean squared error, RMSE)  from including the non-correlated signals, suggesting that development of new sensors for prediction of sewage contamination may not improve what could be done using currently available sensors.
